...
//encryData,需要加密的字段
//commonData,不需要加密的对象
//flag,0表示不需要合并共用数据，1为添加uuid、user_code、token、appid普通参数，2为uuid、appid、token
//waitingDialog,等待框
//callback,返回值
	...
	signHmacSHA1.sign(signTemp, storageKeyName.SIGNKEY, function(sign) {
		...
		mui.ajax(url, {
			...
			success: function(data) {
				...
				//console.log('data.code:' + data.code + ',data.msg:' + data.msg + ',data.data:' + JSON.stringify(data.data) + ',' + url);
				if(data.code == 6) {
					if(waitingDialog != null) {
						waitingDialog.close();
					}
...
function postDataEncry12(encryData, commonData, flag) {
	...
	if(flag == 1) {
		...
		//获取个人信息
		var personaluser_code = store.get(window.storageKeyName.PERSONALINFO).user_code;
		var personalToken = store.get(window.storageKeyName.PERSONALINFO).access_token;
		var comData = {
			uuid: publicParameter.uuid,
			user_code: personaluser_code,
			token: personalToken,
